        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="relative">
              <select
                value={pageFormat.key}
                onChange={(e) => handlePageFormatChange(pageFormatByKey(e.target.value))}
                className="w-full appearance-none bg-bg border border-cleo-border rounded-lg pl-3 pr-9 py-2.5 text-ink text-xs font-cinzel font-bold focus:outline-none focus:border-gold transition-colors cursor-pointer"
              >
                {PAGE_FORMATS.map((p) => (
                  <option key={p.key} value={p.key} className="bg-bg text-ink font-mono">
                    {p.label} · {p.w}×{p.h}mm
                  </option>
                ))}
              </select>
              <svg
                className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gold"
